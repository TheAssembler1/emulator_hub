...
static bool half_carry_uint8_t(uint8_t a, uint8_t b) {
     return (((a & 0xF) + (b & 0xF)) & 0x10) == 0x10;
}

static bool half_carry_uint16_t(uint16_t a, uint16_t b) {
    ...
    if(!cb) {
        switch(fetched_opcode) {
            ...
            case 0x02: // NOTE: LD [BC], A | 1, 8 | ----
                ...
                pc_inc = 1; cycles = 8;
                break;
                // FIXME: next opcode to implement
            ...
            case 0x17: // NOTE: RLA | 1 | 4 | 000C
                c_flag = (cpu->regs.a & 0b10000000) ? SET_FLAG : CLEAR_FLAG;
            ...
            case 0x0C: // NOTE: INC C | 1 | 4 | Z0H-
                hfc_flag = (half_carry_uint8_t(cpu->regs.c, 1)) ? SET_FLAG : CLEAR_FLAG;
                cpu->regs.c++;
        ...
        switch (fetched_opcode) {
            case 0x11: // NOTE: RL C | 2 | 8 | Z00C
                c_flag = (cpu->regs.c & 0b10000000) ? SET_FLAG : CLEAR_FLAG;
                cpu->regs.c = (cpu->regs.c << 1) | (cpu->regs.c >> (8 - 1));
                pc_inc = 2; cycles = 8;
                break;
            case 0x20: // NOTE: SLA B | 2 | 8 | Z00C
                c_flag = (cpu->regs.b & 0b10000000) ? SET_FLAG : CLEAR_FLAG;
                cpu->regs.b = cpu->regs.b << 1;
                z_flag = (!cpu->regs.b) ? SET_FLAG : CLEAR_FLAG;
